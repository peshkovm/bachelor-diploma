...
public abstract class NaiveBayes<T, K> {
    ...
    final HashMap<NaiveBayes.Pair, Integer> likelihood = new HashMap<>(); //concurency not needed final field safe published
    //mapping sentiment to count with given sentiment
    final HashMap<T, Integer> prior_probability = new HashMap<>(); //concurency not needed final field safe published
    //stopwords
    ...
    abstract public void train(T category, Collection<K> vector);
}
