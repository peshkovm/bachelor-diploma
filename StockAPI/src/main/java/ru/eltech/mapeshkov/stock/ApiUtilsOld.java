    ...
    public static class AlphaVantageParser {
        ...
        public static void getAllStockData(final String company) {
            ...
            try {
                ...
                URL url = new URL("https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=" + symbol + "&outputsize=full&apikey=TF0UUHCZB8SBMXDP");

                node = getNodeFromUrl(url);

                Iterator<Map.Entry<String, JsonNode>> fields = getFields(node, "Time Series (Daily)");

                try (PrintWriter writer = new PrintWriter(new FileWriter("C:\\JavaLessons\\bachelor-diploma\\Batch\\src\\test\\resources\\testing batch\\allStockData\\allStockData_" + company + ".txt"), true)) {
